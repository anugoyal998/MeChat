  ...
  return (
    <div className="flex space-y-2 flex-col mt-2">
      {users?.map((p, index) => {
        return (
          user?._id !== p._id && (
            <Card key={index} data={p} flag={flag} setFlag={setFlag} />
          )
        );
      })}
    </div>
  ...
  return (
    <div
      className={`flex justify-between p-3 cursor-pointer ${
        currentChat?._id === data?._id && "bg-hoverBg"
      } hover:bg-hoverBg rounded-lg border shadow-sm`}
      onClick={handleClick}
    >
